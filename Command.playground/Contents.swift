...
let account = Account(accountName: "wtildestar", balance: 1000)
let transactionManager = TransactionManager.shared
transactionManager.addTransactions(command: Deposit(account: account, amount: 199))
transactionManager.addTransactions(command: Withdraw(account: account, amount: 98))
